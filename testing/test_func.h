...
static int my_sum3(int a, int b, int c) { return a + b + c; }
static int my_sum4(int a, int b, int c, int d) { return a + b + c + d; }
...
init_symbols()
{
    ...
    symbols[offset].name = func_names[offset];
    symbols[offset].pointer = &my_sum3;
    ++offset;

    symbols[offset].name = func_names[offset];
    symbols[offset].pointer = &my_sum4;
    ++offset;
...
static void
read_input(size_t sym_start_offset, FILE* test_txt)
{    
    char buffer[128];
    ...
    while (NULL!=fgets(buffer, sizeof(buffer), test_txt)) {
...
static int
call_function_and_return_result(void * addr)
{
...
static int
calculate(char* test_txt_name)
{
    size_t functions_count = init_symbols();

    FILE * test_txt = fopen(test_txt_name, "r");
    if (test_txt == NULL) {
        fprintf(stderr, "Can't open test %s\n", test_txt_name);
        exit(3);
    }
    read_input(functions_count, test_txt);
    fclose(test_txt);
    ...
    int result = call_function_and_return_result(code_buffer);
